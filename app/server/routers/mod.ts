...
import { mods, modStats, users, games, categories, modTags, modImages, modFiles } from '../../db/schema';
...
const getModsInput = z.object({
  ...
  sortBy: z.enum(['downloads', 'rating', 'created', 'updated', 'likes']).default('downloads'),
...
export const modRouter = router({  // Get all mods with filtering and pagination
  getMods: publicProcedure
    .input(getModsInput)
    .query(async ({ ctx, input }) => {
      try {
        const { limit, offset, gameSlug, category, search, sortBy, sortOrder } = input;
        ...
        if (search) {
          conditions.push(
            sql`${mods.title} LIKE ${`%${search}%`} OR ${mods.description} LIKE ${`%${search}%`}`
          );
        }
        ...
        .from(mods)
        .leftJoin(users, eq(mods.authorId, users.id))
  ...
  getModBySlug: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      // Get the full mod details using the ID
  ...
  getModsByAuthor: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      return ctx.db
        ...
        .leftJoin(categories, eq(mods.categoryId, categories.id))
        .leftJoin(modStats, eq(mods.id, modStats.modId))
        .where(and(eq(mods.authorId, input.authorId), eq(mods.isActive, true)))
        .orderBy(desc(mods.createdAt))
        .limit(input.limit)
        .offset(input.offset);
    }),

  // Create a new mod (you might want to add authentication later)
  createMod: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (tags && tags.length > 0) {
        await ctx.db.insert(modTags).values(
          tags.map(tag => ({ modId: newMod.id, tag }))
        );
      }
